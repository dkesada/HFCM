...
def import_and_transform(train_files, test_file, train_path, test_path, classif, sep=',', header=None):
    ...
    for t in train_files:
        with open(f'{train_path}/{classif}/{t}', newline='') as csv_file:
            model_input_train.append(np.array(list(csv.reader(csv_file, delimiter=" "))).astype(np.float))
    with open(f'{test_path}/{classif}/{test_file}', newline='') as csv_file:
        model_input_test = np.array(list(csv.reader(csv_file, delimiter=" "))).astype(np.float)
    ...
    for t in range(len(model_input_train)):
        model_input_train[t] = rescale(min, max)(model_input_train[t])
